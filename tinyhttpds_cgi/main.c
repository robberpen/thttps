...
static int __ssl_cgi_read_from_socket(req_t *req, int to_cgi)
{
	int n;
	char buf[4096];

	n = http_recv(req, buf, sizeof(buf));
	if (n <= 0)
		return n;	//no data from client socket closed
	return write(to_cgi, buf, n);
	
}
static int __ssl_cgi_read_from_cgi(req_t *req, int from_cgi)
{
	int n;
	char buf[4096];
	bzero(buf, sizeof(buf));
	n = read(from_cgi, buf, sizeof(buf));
	if (n <= 0)
		return n;
	return http_send(req, buf, n);
}
static int ssl_cgi_redirect(req_t *req, int from_cgi, int to_cgi)
{
	fd_set rfds;
	struct timeval tv;
	int retval;
	...
	FD_ZERO(&rfds);
	FD_SET(req->sock_fd, &rfds);
	FD_SET(from_cgi, &rfds);
	...
	while (1) {
		fd_set tmp_rfds;
		int have_fd = 0;
		//if ((retval = select(from_cgi + 1, &rfds, NULL, NULL, &tv)) == -1) {
		if ((retval = select(from_cgi + 1, &rfds, NULL, NULL, NULL)) == -1) {
			perror("select() error");
			break;
		}
		__DBG("select return %d\n", retval);
		FD_ZERO(&tmp_rfds);
		if (FD_ISSET(req->sock_fd, &rfds) &&
			__ssl_cgi_read_from_socket(req, to_cgi) > 0)
		{
			FD_SET(req->sock_fd, &tmp_rfds);
			have_fd++;
		}

		if (FD_ISSET(from_cgi, &rfds) &&
			__ssl_cgi_read_from_cgi(req, from_cgi) > 0)
		{
			FD_SET(from_cgi, &tmp_rfds);
			have_fd++;
		}
		if (!have_fd)
			break;
